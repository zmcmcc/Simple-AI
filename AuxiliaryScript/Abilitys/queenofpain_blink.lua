...
--初始数据
local ability = bot:GetAbilityByName('queenofpain_blink')
...
function X.Consider()
    ...
    -- 获取一些必要参数
    local nCastRange = ability:GetSpecialValueInt("blink_range"); --施法范围
    local nCastPoint  = ability:GetCastPoint();	--施法点
    local nAttackPoint = bot:GetAttackPoint(); --攻击点
	...
	local nEnemysHerosInRange = bot:GetNearbyHeroes(nCastRange + 150,true,BOT_MODE_NONE);--获得施法范围内敌人
