...
function X.ConsiderW()
	...
	local WeakestEnemy = GetVulnerableWeakestUnit(true, true, nRadius + 300, bot);
	local WeakestCreep = GetVulnerableWeakestUnit(false, true, nRadius + 300, bot);
...
function X.ConsiderQ()
	...
	then
		...
		then
			if ( bot:WasRecentlyDamagedByAnyHero( 2.0 ) ) 
			then
				return BOT_ACTION_DESIRE_HIGH, locationAoE;
			end
		end
		...
		then
			if ( #nCreeps >= 2 and nMP >0.4) 
			then
				return BOT_ACTION_DESIRE_LOW,bot:FindAoELocation(true, false, bot:GetLocation(), nCastRange, nRadius , 0, 0)
			end
		end
		...
		then
			...
			then
				if ( J.CanCastOnNonMagicImmune( npcEnemy ) and GetUnitToUnitDistance(bot,npcEnemy)<=nRadius)
				then
					return BOT_ACTION_DESIRE_MODERATE,bot:FindAoELocation(true, true, bot:GetLocation(), nCastRange, nRadius , 0, 0)
				end
			end
		end
	else
...
function X.ConsiderR()
	...
	then
		...
		then
			if ( J.CanCastOnNonMagicImmune( WeakestEnemy ) )
			then
				if(WeakestEnemy:GetHealth()<=WeakestEnemy:GetActualIncomingDamage(Damage,DAMAGE_TYPE_MAGICAL))
				then
